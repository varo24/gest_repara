...
<div class="inv-page">
  ...
  <table class="items-table">
    <thead>
      <tr>
        <th>Cantidad</th><th>Código</th><th>Artículo / Descripción</th><th class="r">Precio</th>
        ${!isRecibo ? '<th class="r">IVA</th>' : ''}
        <th class="r">Subtotal</th>
      </tr>
    </thead>
...
const printBulkInvoices = (invs: any[], settings: AppSettings, onNotify: (t: 'success' | 'error' | 'info', m: string) => void) => {
  if (invs.length === 0) { onNotify('info', 'No hay facturas que coincidan con los filtros'); return; }
  const limited = invs.slice(0, 50);
  if (invs.length > 50) onNotify('info', `Se imprimirán las primeras 50 de ${invs.length} facturas`);
  const pages = limited.map(inv => buildInvoicePageHTML(inv, settings)).join('\n');
  const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&display=swap');
*{box-sizing:border-box;margin:0;padding:0}
...
.inv-page{font-family:'Inter',sans-serif;background:white;color:#111;width:210mm;min-height:297mm;padding:12mm 14mm;font-size:10px;position:relative;page-break-after:always}
...
.venc-amount{font-size:13px;font-weight:800;color:#111}
.venc-date{font-size:9px;font-weight:600;color:#555}
.footer{border-top:1px solid #ddd;padding-top:5mm;margin-top:6mm}
.footer-text{font-size:7.5px;color:#888;line-height:1.7}
.footer-legal{font-size:7px;color:#aaa;margin-top:3px}
.stamp{position:absolute;top:80mm;right:20mm;border:3px solid #16a34a;color:#16a34a;padding:4px 12px;font-size:14px;font-weight:900;text-transform:uppercase;letter-spacing:2px;transform:rotate(-15deg);opacity:0.35;border-radius:3px}
.stamp-void{position:absolute;top:80mm;right:20mm;border:3px solid #dc2626;color:#dc2626;padding:4px 12px;font-size:14px;font-weight:900;text-transform:uppercase;letter-spacing:2px;transform:rotate(-15deg);opacity:0.35;border-radius:3px}
  ...
  const frameId = 'bulk-print-frame';
  let frame = document.getElementById(frameId) as HTMLIFrameElement;
  if (frame) frame.remove();
  frame = document.createElement('iframe');
  frame.id = frameId;
...
const Facturacion: React.FC<Props> = ({ settings, customers = [], invoices, inventoryItems = [], onNotify, onSaveCustomer, onBack }) => {
  ...
  const [bulkCobradas, setBulkCobradas]     = useState(true);
  const [bulkPendientes, setBulkPendientes] = useState(true);
  const [bulkAnuladas, setBulkAnuladas]     = useState(false);
  const [bulkOnlyFac, setBulkOnlyFac]       = useState(false);
  const [bulkOnlyRec, setBulkOnlyRec]       = useState(false);
  const [bulkWarranty, setBulkWarranty]     = useState(false);
  ...
  const getInvNum = (n: string) => { const p = (n || '').split('-'); return parseInt(p[p.length - 1], 10) || 0; };
  const bulkFiltered = useMemo(() => {
    const numFrom = bulkFrom ? parseInt(bulkFrom, 10) : 0;
    const numTo   = bulkTo   ? parseInt(bulkTo, 10)   : Infinity;
    const activeStatuses = [
      ...(bulkCobradas   ? ['cobrada']   : []),
      ...(bulkPendientes ? ['pendiente'] : []),
      ...(bulkAnuladas   ? ['anulada']   : []),
    ];
    return visibleInvoices.filter(inv => {
      if (bulkOnlyFac && !bulkOnlyRec && !(inv.invoiceNumber || '').startsWith('FAC-')) return false;
      if (bulkOnlyRec && !bulkOnlyFac && !(inv.invoiceNumber || '').startsWith('REC-')) return false;
      const num = getInvNum(inv.invoiceNumber || '');
      if (bulkFrom && num < numFrom) return false;
      if (bulkTo   && num > numTo)   return false;
      if (bulkDateFrom && inv.date < bulkDateFrom) return false;
      if (bulkDateTo   && inv.date > bulkDateTo)   return false;
      if (activeStatuses.length > 0 && !activeStatuses.includes(inv.status)) return false;
      return true;
    }).sort((a, b) => (a.invoiceNumber || '').localeCompare(b.invoiceNumber || ''));
  }, [visibleInvoices, bulkFrom, bulkTo, bulkDateFrom, bulkDateTo, bulkCobradas, bulkPendientes, bulkAnuladas, bulkOnlyFac, bulkOnlyRec]);
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-200">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowBulkPrint(true)} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-black uppercase hover:bg-slate-50 transition-all shadow-sm">
            <Layers size={13}/> Imprimir en bloque
          </button>
      ...
      {/* Modal impresión en bloque */}
      {showBulkPrint && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-7 space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Impresión masiva</p>
                <p className="text-xl font-black text-slate-900">Imprimir en bloque</p>
              </div>
              <button onClick={() => setShowBulkPrint(false)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-all"><X size={16}/></button>
            </div>

            {/* Rango de facturas */}
            <div>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Rango de número de factura</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[9px] text-slate-400 mb-1">Desde nº</label>
                  <input type="number" min="1" value={bulkFrom} onChange={e => setBulkFrom(e.target.value)} placeholder="1"
                ...
                <div>
                  <label className="block text-[9px] text-slate-400 mb-1">Hasta nº</label>
                  <input type="number" min="1" value={bulkTo} onChange={e => setBulkTo(e.target.value)} placeholder="sin límite"
                    className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"/>
                </div>
              </div>
            </div>
            ...
            {/* Filtro por fecha */}
            <div>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Filtro por fecha</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[9px] text-slate-400 mb-1">Desde</label>
                  <input type="date" value={bulkDateFrom} onChange={e => setBulkDateFrom(e.target.value)}
                    className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"/>
                </div>
                <div>
                  <label className="block text-[9px] text-slate-400 mb-1">Hasta</label>
            ...
            <div className="grid grid-cols-2 gap-6">
              {/* Estado */}
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Estado</p>
                <div className="space-y-2">
                  {([['Cobradas', bulkCobradas, setBulkCobradas], ['Pendientes', bulkPendientes, setBulkPendientes], ['Anuladas', bulkAnuladas, setBulkAnuladas]] as const).map(([label, val, setter]) => (
                    <label key={label} className="flex items-center gap-2 cursor-pointer group">
                      <input type="checkbox" checked={val} onChange={e => setter(e.target.checked)} className="w-4 h-4 accent-blue-600"/>
              ...
              {/* Opciones */}
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Opciones de impresión</p>
                <div className="space-y-2">
                  {([['Solo FAC- (con IVA)', bulkOnlyFac, setBulkOnlyFac], ['Solo REC- (sin IVA)', bulkOnlyRec, setBulkOnlyRec], ['Incl. certificado garantía', bulkWarranty, setBulkWarranty]] as const).map(([label, val, setter]) => (
                    <label key={label} className="flex items-center gap-2 cursor-pointer group">
                      <input type="checkbox" checked={val} onChange={e => setter(e.target.checked)} className="w-4 h-4 accent-blue-600"/>
                      <span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 leading-tight">{label}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-slate-50 rounded-xl px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-slate-700">
                  {Math.min(bulkFiltered.length, 50)} factura{Math.min(bulkFiltered.length, 50) !== 1 ? 's' : ''} seleccionadas
                  {bulkFiltered.length > 50 && <span className="text-amber-600 ml-1">(máx. 50 por impresión)</span>}
                </span>
              </div>
              <span className="text-sm font-black text-slate-900">
                {fmtMoney(bulkFiltered.slice(0, 50).reduce((s, i) => s + (i.total || 0), 0))}
              </span>
            </div>

            {/* Botones */}
            <div className="flex gap-3">
              <button onClick={() => setShowBulkPrint(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl text-xs font-black uppercase hover:bg-slate-200 transition-all">
                Cancelar
              </button>
              <button
                onClick={() => { printBulkInvoices(bulkFiltered, settings, onNotify); setShowBulkPrint(false); }}
                disabled={bulkFiltered.length === 0}
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-900 text-white rounded-xl text-xs font-black uppercase hover:bg-black transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <Printer size={13}/> Imprimir {Math.min(bulkFiltered.length, 50)} facturas
              </button>
            </div>
          </div>
        </div>
